...
# -----------------------------
# 🤖 AI Response Models and Dependencies (pydantic-ai v2+ syntax)
# -----------------------------
...
def init_rag_db():
    ...
    # خواندن اسناد از فایل
    try:
        with open("sample_docs.txt", "r", encoding="utf-8") as f:
            sample_docs = [line.strip() for line in f if line.strip()]
    except FileNotFoundError:
        # Default sample documents if file not found
        sample_docs = [
            "صندوق سام یک صندوق درآمد ثابت است که در بازار سرمایه فعالیت می‌کند.",
            "صندوق رویین متمرکز بر سرمایه‌گذاری در صنعت فلزات اساسی است.",
            "صندوق قلک برای سرمایه‌گذاری محافظه‌کارانه طراحی شده است.",
            "صندوق سورنافود در حوزه صنایع غذایی سرمایه‌گذاری می‌کند."
        ]
...
# -----------------------------
# pydantic-ai v2+ agent definition
# -----------------------------
financial_agent = Agent(
    ...
    instructions=(
        "You are Sourena, a specialized financial assistant for an Iranian investment company.\n"
        "STRICT INSTRUCTIONS:\n"
        "1. Analyze if the user's question is about finance, investments, funds, markets, economics or Surena asset management company.\n"
        "2. If YES (financial or Surena company): Use the provided document context as the primary source to generate a comprehensive, accurate answer in Persian (max 2-3 sentences). Combine information from all relevant documents to provide a complete response, staying as close as possible to the document content.\n"
        "3. If NO (non-financial and not about Surena company): Use EXACTLY this Persian message: \"من دستیار هوش مصنوعی سبدگردان سورنا هستم و فقط در زمینه مالی و سرمایه گذاری می توانم به شما کمک کنم.\"\n"
...
@financial_agent.instructions
async def add_rag_context(ctx: RunContext[FinancialAgentDeps], message: str) -> str:
    # This hook is not strictly necessary for this use-case, but you could add user info, etc.
    return ""
...
# -----------------------------
# 🤖 Handlers: AI Chat with Enhanced RAG (pydantic-ai v2+)
# -----------------------------
async def handle_ai_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with httpx.AsyncClient() as http_client:
            deps = FinancialAgentDeps(api_key=OPENAI_API_KEY, http_client=http_client)
            result = await financial_agent.run(augmented_message, deps=deps)
